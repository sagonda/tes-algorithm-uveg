...
# -*- coding: utf-8 -*-
"""
Created on Mon Oct 11 20:55:47 2021
...
try:
    ...
    if os.path.isfile('/gws/nopw/j04/esacci_lst/UV/output_uveg/'+year+'/'+month+'/'+year+month+'.csv'):
        print('Existe el archivo de rutas')
        INIT , hour = rt.read_csv_files('/gws/nopw/j04/esacci_lst/UV/output_uveg/'+year+'/'+month+'/', year, month)
    else:
        hour = '0000'
...
for day_ in range(int(INIT), num_days+1):
    try:
        ...
        if not os.path.exists('/gws/nopw/j04/esacci_lst/UV/output_uveg/'+year+'/'+month+'/'+day+'/'):
            os.makedirs('/gws/nopw/j04/esacci_lst/UV/output_uveg/'+year+'/'+month+'/'+day+'/')
        
        #----------Read NDVI----------
        ndvi_lat, ndvi_lon, ndvi = rt.read_ndvi_file('/gws/nopw/j04/esacci_lst/UV/data/ndvi/', year, month)
        
        #***********init time********
        start_time = time.time()
        
        #----------Read MYD03 files------
        lista_files_Myd03 = rt.read_myd03_files('/neodc/modis/data/MYD03/collection61/',  year, month, day)
        
        #---------Read Myd021km files....
        lista_files_Myd02 = rt.read_myd021_files('/neodc/modis/data/MYD021KM/collection61/',  year, month, day)
        
        #---------Read Myd35 files....
        lista_files_Myd35 = rt.read_myd021_files('/neodc/modis/data/MYD35_L2/collection61/',  year, month, day)
        ...
        mask_sea_land_era5 = rt.extract_mask_land('/badc/ecmwf-era5/data/invariants/')
        ...
        for hours_list in range(INIT_HOURS, len(hours)): 
            try:     
                ...
                if os.path.exists('/badc/ecmwf-era5/data/oper/an_ml/'+year+'/'+month+'/'+day+'/'):
                    file_data = 'ecmwf-era5_oper_an_ml_'
                    temp_era, hum_E, lat_Nc, lon_Nc = rt.extract_vars_era5('/badc/ecmwf-era5/data/oper/an_ml/', year, month, day, mask_sea_land_era5, hours, hours_list, file_data)
                else:
                    file_data = 'ecmwf-era51_oper_an_ml_'
                    temp_era, hum_E, lat_Nc, lon_Nc = rt.extract_vars_era5('/badc/ecmwf-era51/data/oper/an_ml/', year, month, day, mask_sea_land_era5, hours, hours_list, file_data)
                
                if os.path.exists('/badc/ecmwf-era5/data/oper/an_sfc/'+year+'/'+month+'/'+day+'/'):
                    file_data_ = 'ecmwf-era5_oper_an_sfc_'
                    t2m, skt, d2m, msl1, level = rt.extract_vars2m_era5('/badc/ecmwf-era5/data/oper/an_sfc/',year, month, day, mask_sea_land_era5, hours, hours_list, file_data_)
                    
                else:
                    file_data_ = 'ecmwf-era51_oper_an_sfc_'
                    t2m, skt, d2m, msl1, level = rt.extract_vars2m_era5('/badc/ecmwf-era51/data/oper/an_sfc/',year, month, day, mask_sea_land_era5, hours, hours_list, file_data_)
            ...
            for i_modis in range(rep):
                try:
                    ...
                    if os.path.isfile('/gws/nopw/j04/esacci_lst/UV/output_uveg/'+year+'/'+month+'/'+day+'/'+'TES_'+year+month+day+'_'+str(date_modis)+'_'+str(toma)+'.nc'):
                    ...
                    path_nc_file = '/gws/nopw/j04/esacci_lst/UV/output_uveg/'
                    ...
                    bt, radtotal, radup, raddown, tautotal  = rt.call_rttov('/gws/nopw/j04/esacci_lst/UV/software/rttov12/rtcoef_rttov12/', datetimes, angles, \
                    ...
                    rt.create_nc_outfile('/gws/nopw/j04/esacci_lst/UV/output_uveg/', year, month, day, date_modis, dimension_original,\
                                    latitud_int, longitud_int, Ts_1_int, e_1_int, mask_original_modis, toma,  errTs_int, err_e29_int, err_e31_int, err_e32_int, z_int)
                    
                    rt.write_csv_files('/gws/nopw/j04/esacci_lst/UV/output_uveg/'+year+'/'+month+'/', 'TES_'+year+month+day+'_'+date_modis+'_'+toma+'.nc', year, month)
