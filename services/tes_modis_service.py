...
class tes_modis_services():
    ...
    def tes_modis(self):
        ...
        try:
            ...
            e[:] = 0.97 # shape(2030,1354)
            # print(BT.shape)
            #print('** ESTIMANDO LST Y EMISIVIDAD CON - TES ALGORITHM - **')
            ...
            # Tengo valores menores de 0 porque hay radiancias con valores menores a 2
            ...
            R[:] = [rad[i,:] - (1-e[i,:])*self.ldown[i,:] for i in range(3)]
            T[:] = [K2[i]/np.log(K1[i]/R[i,:]+1) for i in range(3)] 
            Ts[:] = np.max(T[:,:],axis=0)# crear variable para la temperatura anterior
            BT[:] = [K1[i]/(np.exp(K2[i]/Ts[0,:])-1) for i in range(3)]
