...
import numpy   as np
from services.new_array_service import NewArrayService

class CreateNcOutfileService():
    ...
    def create_nc_outfile(self):
        try:
            ...
            lat[:]       = self.latitud
            lon[:]       = self.longitud
            lst[:]       = NewArrayService(self.Ts, mask1_, self.dimension_original, data_type=np.uint16).new_array()
            e29[:]       = NewArrayService(self.e[0,:], mask1_, self.dimension_original, data_type=np.uint8).new_array()
            e31[:]       = NewArrayService(self.e[1,:], mask1_, self.dimension_original, data_type=np.uint8).new_array()
            e32[:]       = NewArrayService(self.e[2,:], mask1_, self.dimension_original, data_type=np.uint8).new_array()
            lst_error[:] = NewArrayService(self.errTs, mask1_, self.dimension_original, data_type=np.uint8).new_array()
            e29_error[:] = NewArrayService(self.err_e29, mask1_, self.dimension_original, data_type=np.uint16).new_array()
            e31_error[:] = NewArrayService(self.err_e31, mask1_, self.dimension_original, data_type=np.uint16).new_array()
            e32_error[:] = NewArrayService(self.err_e32, mask1_, self.dimension_original, data_type=np.uint16).new_array()
            z_[:]        = NewArrayService(self.z, mask1_, self.dimension_original, data_type=np.uint8).new_array()
