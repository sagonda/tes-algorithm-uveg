...
from utilities.utilities_extract import utilities_extraction
        ...
        for hours_list in range(INIT_HOURS, 2):  # len(hours)):
            ...
            try:
                ...
                if os.path.exists(input_path_images_ecmwfera5_an_ml+year+'/'+month+'/'+day+'/'):
                    ...
                    temp_era, hum_E, lat_Nc, lon_Nc = utilities_extraction.extract_vars_era5(
                ...
                if os.path.exists(input_path_images_ecmwfera5_an_sfc+year+'/'+month+'/'+day+'/'):
                    ...
                    t2m, skt, d2m, msl1, level = utilities_extraction.extract_vars2m_era5(
            ...
            for i_modis in range(rep):
                try:
                    ...
                    lat_Modis, lon_Modis, mask_land_modis, mask_original_modis = utilities_extraction.extract_index_image(
                        lat_Myd03[i_modis], lon_Myd03[i_modis], mask_sea_land_modis[i_modis], dimension)

                    ndvi_d = utilities_extraction.extract_ndvi(
                    ...
                    index_Era, lat, lon = utilities_extraction.extract_index_modis_and_era(
                        lat_Nc, lon_Nc, lat_Modis, lon_Modis, mask_sea_land_era5)

                    date_era5 = year+month+day

                    h_ = utilities_extraction.extract_height(
                        h_03, mask_land_modis, cloud_mask_flag)
                    print('Pixeles a procesar:', h_.shape)

                    z = utilities_extraction.extract_zenith(
                        s_Z, mask_land_modis, cloud_mask_flag)
                    t_ = utilities_extraction.extract_temperature(
                        temp_era, index_Era)
                    he = utilities_extraction.extract_humidity(
                        hum_E, index_Era)
                    t2, sk, p2m, q2m = utilities_extraction.extract_param_2m(
                    ...
                    datetimes, angles, surftype, surfgeom, s2m, skin, simplecloud, clwscheme, icecloud, \
                        zeeman, p, t = create_profiles_services(
                            dimension_modis_ravel, z, lat_Modis, lon_Modis, h_, p2m, t2, q2m, sk, level, t_, year, month, day).create_profiles()

                    nc_file = net.Dataset(output_path_images_uveg+year+'/'+month+'/'+day+'/'+'TES_' +
                                          year+month+day+'_'+date_modis+'_'+toma+'.nc', mode='w', format='NETCDF4')
                    nc_file.close()

                    print('Se ha creado el archivo temporal:', output_path_images_uveg+year+'/' +
                          month+'/'+day+'/'+'TES_'+year+month+day+'_'+date_modis+'_'+toma+'.nc')
                    bt, radtotal, radup, raddown, tautotal = call_rttov_services(
                        input_path_images_rtcoef_rttov12, datetimes, angles, surfgeom, surftype, s2m, skin, simplecloud, clwscheme, icecloud, zeeman, p, t, he).call_rttov()

                    Lbb29, Lbb31, Lbb32 = processUveg(sk).variables()

                    Lup_29, Lup_31, Lup_32, Ldown_29, Ldown_31, Ldown_32 = utilitiesUveg.chance_units(utilitiesUveg.consA, utilitiesUveg.consB, bt, radtotal, radup, raddown, tautotal, utilitiesUveg.emissivity,
                                                                                                      utilitiesUveg.nOnda29, utilitiesUveg.nOnda31, utilitiesUveg.nOnda32, utilitiesUveg.lonOnda29, utilitiesUveg.lonOnda31, utilitiesUveg.lonOnda32, Lbb29, Lbb31, Lbb32)
                    ...
                    ldown1 = utilitiesUveg.create_array_bidimentional(
                        3, dimension_modis_ravel)
                    ...
                    Ts2, e2, rad2, BT2,  R2, erad2 = tes_modis_services(
                        utilitiesUveg.lo, lup, ldown2, trans, radiance2, z=z.ravel(), aux=False, recal=False).tes_modis()
                    ...
                    ###########TES UVEG###################
                    Ts_1, e_1, BT_, rad_1, R_1, erad_1 = tes_modis_services(
                        utilitiesUveg.lo, lup, ldown, trans, rad_recal, z=z.ravel(), aux=False, recal=True).tes_modis()
                    Ts_1 = np.round(Ts_1, 2)
                    Ts_1_int = utilitiesUveg.packed_value(
                        Ts_1, 0.0, 0.02, data_type=np.uint16)

                    e_1 = np.round(e_1, 3)
                    e_1_int = utilitiesUveg.packed_value(
                        e_1, 0.49, 0.002, data_type=np.uint8)

                    errTs = np.round(errTs, 2)
                    errTs_int = utilitiesUveg.packed_value(
                        errTs, 0.0, 0.04, data_type=np.uint8)

                    err_e29 = np.round(err_e29, 4)
                    err_e29_int = utilitiesUveg.packed_value(
                        err_e29, 0.0, 0.0001, data_type=np.uint16)

                    err_e31 = np.round(err_e31, 4)
                    err_e31_int = utilitiesUveg.packed_value(
                        err_e31, 0.0, 0.0001, data_type=np.uint16)

                    err_e32 = np.round(err_e32, 4)
                    err_e32_int = utilitiesUveg.packed_value(
                        err_e32, 0.0, 0.0001, data_type=np.uint16)

                    z = np.round(z, 2)
                    z_int = utilitiesUveg.packed_value(
                        z, 0.0, 0.5, data_type=np.uint8)

                    latitud_int = (latitud*10000).astype(np.int32)
                    longitud_int = (longitud*10000).astype(np.int32)
                    if os.path.exists(output_path_images_uveg+year+'/'+month+'/'+day+'/'+'TES_'+year+month+day+'_'+date_modis+'_'+toma+'.nc'):
                        os.remove(output_path_images_uveg+year+'/'+month+'/'+day+'/' +
